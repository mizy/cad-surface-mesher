...
from hybrid_proxy_fusion import run_hybrid_proxy_fusion
...
def run_pipeline(args: argparse.Namespace, status: dict[str, Any]) -> int:
    ...
    closure_visual_drift = silhouette_drift_from_meshes(
        candidate_points,
        candidate_faces,
        stage2_points,
        stage2_faces,
    ...
    update_run_status(args, status, "hybrid_fusion_started", voxel_pitch=pitch_status(args))
    hybrid_result = run_hybrid_proxy_fusion(
        candidate_points,
        candidate_faces,
        candidate_sources,
        stage2_points,
    ...
    hybrid_mesh = read_surface(hybrid_candidate_path)
    hybrid_points = np.asarray(hybrid_mesh.points, dtype=np.float64)
    hybrid_faces = triangle_faces(hybrid_mesh)
    update_run_status(
        args,
        status,
        "hybrid_fused_candidate_written",
        outputs={"hybrid_fused_candidate_vtp": str(hybrid_candidate_path)},
        metrics={"hybrid_fused_candidate": topology_brief(hybrid_result["final_metrics"])},
        voxel_pitch=pitch_status(args),
    )

    hybrid_visual_drift = silhouette_drift_from_meshes(
        candidate_points,
        candidate_faces,
        hybrid_points,
        hybrid_faces,
    ...
    report = build_report(
        args,
        {
            ...
            "hybrid_fused_candidate": hybrid_result["final_metrics"],
        },
        extraction,
