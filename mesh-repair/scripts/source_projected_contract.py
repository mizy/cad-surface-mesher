...
def watertight_shell_policy(target_name: str) -> dict[str, Any]:
    ...
    return {
        ...
        "local_patch_policy_applicability": (
...
def build_projection_gates(
    ...
    gates["opening_policy_resolved"] = {
        ...
        "value": shell_policy.get("local_patch_policy_applicability"),
    ...
    gates.update({
        ...
        "accepted_path_is_source_projected_candidate": gate(
            ...
            "accepted output must be the projected candidate, not the raw proxy or hybrid alternative",
        ),
...
def local_policy_diagnostics(items: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return [
        {
            **row,
            "blocking": False,
            "applicability": "hybrid_alternative_only",
        }
        for row in items
    ]
