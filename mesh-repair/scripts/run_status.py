...
def partial_outputs(output_dir: Path) -> dict[str, Any]:
    paths = {
        ...
        "hybrid_fused_candidate_vtp": output_dir / "hybrid_fused_candidate.vtp",
    }
    ...
    rejected = next(
        (
            path
            for path in (
                paths["source_projected_watertight_candidate_vtp"],
                paths["hybrid_fused_candidate_vtp"],
            )
            if path.exists()
        ),
...
def ignored_partial_outputs(outputs: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if outputs.get("hybrid_fused_candidate_vtp"):
        rows.append({
            "path": outputs["hybrid_fused_candidate_vtp"],
            "kind": "failed_hybrid_candidate",
...
def render_failure_html(report: dict[str, Any]) -> str:
    ...
    return "\n".join([
        "<!doctype html>",
        "<html><head><meta charset=\"utf-8\"><title>Hybrid Mesh Repair Run Failed</title></head>",
        "<body>",
        "<h1>Hybrid Mesh Repair Run Failed</h1>",
