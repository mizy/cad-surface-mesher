...
from mesh_metrics import edge_topology, face_component_labels, mesh_report
...
def _closest_source_points(
    ...
    # PyVista dispatches this batch query through VTK's static cell locator in
    # compiled code.  It is equivalent to calling FindClosestPoint for every
    # vertex, but avoids a Python callback for each proxy point.
    import pyvista as pv

    packed = np.empty((source_faces.shape[0], 4), dtype=np.int64)
    packed[:, 0] = 3
    packed[:, 1:] = source_faces
    surface = pv.PolyData(source_points, packed.ravel())
    face_ids, closest_points = surface.find_closest_cell(
    ...
    distances = np.linalg.norm(
        np.asarray(query_points, dtype=np.float64) - closest_points,
        axis=1,
    )
...
def project_closure_to_source(
    ...
    *,
    ...
    intersections = {
        "status": "not_computed",
        "passed": False,
        "reason": "libigl/CGAL certification belongs to the caller acceptance path",
    }
