...
# sent to the opening-policy packet.  These thresholds classify source defects;
# they do not select local topology-edit operators.
...
BOUNDARY_THRESHOLD_DESCRIPTIONS = {
    "small_min_exterior_confidence": "minimum exterior support for deterministic small-opening classification",
    ...
    "small_min_compactness": "minimum 4*pi*projected_area/perimeter^2 for small-opening classification",
    "small_max_component_boundary_count": "maximum boundary-region count on a component for automatic small-opening classification",
    "small_polygon_max_edge_count": "maximum edge count for an isolated missing-polygon classification exception",
    "small_polygon_max_diameter_bbox_ratio": "maximum bbox-normalized diameter for a missing-polygon classification exception",
    "small_polygon_max_projected_area_bbox_ratio": "maximum bbox-normalized area for a missing-polygon classification exception",
    "small_polygon_max_diameter_local_edge_ratio": "maximum diameter in boundary-edge lengths for a missing-polygon classification exception",
...
def classify_boundary_regions(
    ...
    *,
    ...
    Simple loops with exterior evidence and compatible loop pairs are recorded as
    closure evidence.  The only automatic topology owner is the sealed-exterior
    TSDF scaffold; this classifier never requests local welding or patch insertion.
    """
    ...
    return rows, {
        ...
        "rules": [
            ...
            "near-coincident compatible loop pairs are closure evidence, not a request for a local seam edit",
            "small-opening classification requires simple-loop geometry plus exterior-component evidence",
            ...
            "internal or fragment component perimeters are never sent to a local hole filler",
        ],
    }
...
def classify_one(
    ...
    if all(semantic_checks.values()):
        return {
            **base,
            ...
            "operator": "sealed_exterior_tsdf_after_cap_decision",
    ...
    if component_perimeter and component_perimeter_checks["diameter_dominates_component"]:
        ...
        reason = "subsemantic_scale_loop_requires_review"
    else:
    ...
    return {
        **base,
        ...
        "operator": "skill_semantic_review",
        "decision_evidence": decision_evidence,
    }
