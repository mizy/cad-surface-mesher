...
from agent_repair_contract import (  # noqa: E402
    ...
    combined_acceptance_gates,
    decision_schema,
    deterministic_transaction_log,
...
class AgentRepairContractTest(unittest.TestCase):
    ...
    def test_transaction_log_is_derived_from_deterministic_report(self) -> None:
        geometry_report = {
            "stages": {"closure_proxy": {"path": "/tmp/closure.vtp"}},
            "source_projection": {"status": "computed"},
        }
        rows = deterministic_transaction_log(
            geometry_report,
            {
                "input_geometry_hash": "input-hash",
                "candidate_geometry_hash": "candidate-hash",
            },
        )
        self.assertEqual(
            [row["transaction_id"] for row in rows],
            ["tx_tsdf_closure", "tx_source_projection"],
        )
