...
- `.agents/skills/mesh-watertight-repair/scripts/solid_triangle_raster.py` provides conservative solid-triangle depth,
  silhouette, and first-hit face-ID rasterization and is the strongest current renderer base.
- `two_stage_watertight_remesh.py` and `opening_policy_visuals.py` provide registered exterior and
  focused-region evidence without creating a second AI entrypoint.
- `boundary_classification.py`, `watertight_issue_markers.py`, and
...
- The active Skill owns semantic review; deterministic geometry scripts never launch Codex.
